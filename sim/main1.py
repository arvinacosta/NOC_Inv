...
#NEW Working API
#curl -k "URL/APIItems"
@app.get("/APIitemsX", response_class=JSONResponse)
...
async def read_items(request: Request):
    ...
    return JSONResponse(content={"items": items, "distinct_word": distinct_word, "kb_patch": kb_patch})


# Route to serve the HTML page using Jinja2 template
@app.get("/items", response_class=HTMLResponse)
async def get_html(request: Request):
    # Fetch data from the API
    items_data = await fetch_items_from_api()
    
    # Render the HTML template and pass the fetched data to it
    return templates.TemplateResponse("main_tem2.html", {"request": request, "items": items_data})


# Helper function to fetch items from the API
async def fetch_items_from_api():
    # Simulate a database fetch or use an actual API call to get data
    response = await read_items()
    return response['items']

    
# Route to serve the JSON data from the database (API)
@app.get("/APIitems", response_class=JSONResponse)
async def read_items():
    ...
    return {"items": items, "distinct_word": distinct_word, "kb_patch": kb_patch}



#NEW Working
@app.get("/itemsX", response_class=HTMLResponse)
async def read_items(request: Request):
    distinct_word = os.getenv("DistinctWord")
    kb_patch = os.getenv("KBPatch")
    ...
    # Return the response with the template  item_maint_good   main_item
    return templates.TemplateResponse("main_item2.html", {
        "request": request,
