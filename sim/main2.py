...
class Item(BaseModel):
    ...
    status: str
    switch: str
    timestamp: datetime  # Ensure timestamp is included as a datetime field


# ===================================================================================================
@app.get("/", response_class=HTMLResponse)
async def root(request: Request):
    return templates.TemplateResponse("main_item2.html", {"request": request})
...
async def get_items():
    ...
    # ==============================================
    # Prepare the headers
    headers = ['ID', 'Serial Number', 'Location', 'Brand', 'Model', 'Description', 'itypes', 'MAC Address', 'Remarks', 'Status', 'Switch', 'Timestamp']

    # Extract the values for each row (data)
    rows = [[item['id'], item['serial_number'], item['location'], item['brand'], item['model'], item['description'], item['itypes'], item['macadd'], item['remarks'], item['status'], item['switch'], item['timestamp']] for item in items]
...
async def update_item(id: int, item: Item):
    ...
    item.timestamp = datetime.now()
    item.timestamp = item.timestamp.strftime("%Y-%m-%d %H:%M:%S")
    #print("Received time:", item.timestamp)

    cursor.execute('''
        UPDATE items
        SET serial_number = ?, location = ?, brand = ?, model = ?, description = ?, itypes = ?, macadd = ?, remarks = ?, status = ?, switch = ?, timestamp = ?
        WHERE id = ?
    ''', (item.serial_number, item.location, item.brand, item.model, item.description, item.itypes,
          item.macadd, item.remarks, item.status, item.switch, item.timestamp, id))
    conn.commit()
    conn.close()
...
async def save_item(item: Item):
    ...
    if item.id:
        ...
        cursor.execute('''
            ...
            SET serial_number = ?, location = ?, brand = ?, model = ?, description = ?, itypes = ?, macadd = ?, remarks = ?, status = ?, switch = ?, timestamp = ?
            WHERE id = ?
        ''', (item.serial_number, item.location, item.brand, item.model, item.description, item.itypes, item.macadd, item.remarks, item.status, item.switch, item.timestamp, item.id))
    else:
